...
impl RabbitMQService {
    ...
    pub async fn publish_for_user<T: Serialize>(
        &self,
        task_type: &str,
        user_id: &str,
        message: &T,
        ...
        // Publish the message with a routing key that includes the user ID
        let routing_key = format!("task.{}.{}", task_type, user_id);
        let payload = message_json.as_bytes();
        ...
        log::info!("Published message with routing key: {}", routing_key);
